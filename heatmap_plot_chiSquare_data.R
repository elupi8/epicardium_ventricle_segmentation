...
dt = fread(file.path(getwd(), "heat_map_chi_sq_PLOT_HMinverted.csv"), header = T)  #name of the input file
...
dt$heart <- factor(dt$heart, levels = c("PE_1", "PE_2", "Epi_1", "Epi_2", "HM_1", "HM_2")) #reorder the factors for the final graph
...
pdf( file.path(output.Directory, paste0("heat_map_chi_sq_INVERTED_hm",".pdf") ), width = 10, height = 5) 
g
dev.off()
